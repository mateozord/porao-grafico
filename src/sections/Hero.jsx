...
export default function Hero() {
  const parallaxRef = useScrollParallax({ factor: 0.06, max: 14 })
  const cracksRef = useScrollParallax({ factor: 0.025, max: 8 })
  ...
  return (
    ...
    >
      ...
      <div
        ref={cracksRef}
        className="hero-parallax wall-cracks absolute inset-0 opacity-50"
      />
      <div className="relative z-10 mx-auto grid w-full max-w-7xl items-center gap-10 lg:min-h-[calc(100svh-120px)] lg:grid-cols-[1.05fr_.7fr]">
        <div>
          <Reveal distance={16} duration={600}>
            ...
            <h1 className="max-w-5xl font-display text-5xl uppercase leading-[0.9] text-bone sm:text-7xl md:text-8xl lg:text-9xl">
              Arte pra banda, evento ou lançamento que não quer parecer coisa
              de agência.
            </h1>
          </Reveal>

          <Reveal distance={14} duration={550} delay={130}>
          ...
          <Reveal distance={12} duration={550} delay={260}>
